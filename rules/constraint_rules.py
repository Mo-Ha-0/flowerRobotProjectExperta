...
أربع قواعد (salience=10 > salience=0 لقواعد التوسيع):
  ...
  3. wrong_type_violation    — محاولة تفريغ باقة لا تناسب الجناح → سحب العقدة
  4. out_of_bounds_violation — موقع خارج الشبكة → سحب العقدة

آلية العمل:
  - القواعد 1، 2، 4 تَسحب العقدة المنتهِكة (retract) لأن الحالة مستحيلة
  - المرور فوق جناح مع حمولة لنوع آخر ليس انتهاكاً بحد ذاته
"""
...
def _wrong_type_at_pavilion(action: str, rx: int, ry: int,
                            bouquets: list, pavilions: list) -> bool:
    """
    True إذا:
      - العقدة تمثل محاولة تفريغ غير صحيحة
      - الروبوت في موقع جناح
      - يحمل باقات
      - توجد باقة من نوع مختلف عن نوع هذا الجناح
    """
    if not action.startswith("invalid_unload"):
        return False
    if not bouquets:
        return False
    ...
    if pav is None:
        return False
    ft = pav["flower_type"]
    return any(bt != ft for bt, bc in bouquets)
...
class ConstraintRulesMixin:
    ...
    # ════════════════════════════════════════════════════════
    # الانتهاك: محاولة تفريغ باقة لا تناسب نوع الجناح
    # الإجراء : سحب العقدة + تسجيل الانتهاك
    @Rule(
        AS.node << SearchNode(
            node_id   = MATCH.nid,
        ...
        TEST(lambda action, rx, ry, bq, pavs: _wrong_type_at_pavilion(action, rx, ry, bq, pavs)),
        salience = 10,
    )
    def wrong_type_violation(self, node, nid, action, rx, ry, bq, pavs, cost, depth):
        """محاولة تفريغ باقة لا تناسب الجناح — سحب العقدة"""
